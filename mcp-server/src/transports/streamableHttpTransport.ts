...
export class StreamableHTTPTransport {
  ...
  async handleRequest(req: Request, res: Response): Promise<void> {
    try {
      // Set appropriate headers for JSON-RPC
      ...
      // Validate JSON-RPC request using Zod
      ...
      // Process the validated JSON-RPC request
      ...
      try {
        ...
        const errorResponse = createJsonRpcError(
          ...
          responseValidationError instanceof z.ZodError ? responseValidationError.errors : responseValidationError,
          validatedRequest.id
        );
  ...
  /**
   * Process JSON-RPC request directly with the MCP server
   */
  ...
  private async handleToolsList(): Promise<any> {
    // Get tools from the server's registered handlers
    const tools = [
      {
        name: 'echo',
...
export function createStreamableHTTPTransport(server: Server, toolHandlers?: Map<string, (args: any) => Promise<any>>) {
